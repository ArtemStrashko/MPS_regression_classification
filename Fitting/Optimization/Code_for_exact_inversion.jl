...
function valid_loss(W_tensors, phys_indices, parameters=parameters)
	...
	sample = parameters["sample"]
    name = "validation_data_a_$(a)_D_$(D)"
	name *= "_$(N)_points"
    path = "./test_train_data/"*name*".jld"
    test_data = jldopen(path, "r") do file
        read(file, name)
    ...
    phi_valid, labels_valid = create_Phi_vectors_for_all_data_points(test_data)
    valid_loss_val = loss(W_tensors, phi_valid, labels_valid, phys_indices)
...
#         w_init = rand(729)
    ...
    return MPS(W_tensor, phys_indices; cutoff=0, maxdim=100) 
